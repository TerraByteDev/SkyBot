...
module.exports = async (client, message) => {
    ...
    let getXpfromDB = config.xpPerMessage ?? 16;
    let getCooldownfromDB = config.cooldown ?? 1000;
    ...
    try {
        ...
        if (talkedRecently.get(message.author.id)) {
            ...
            if (level.xp >= nextXP) {
                ...
                let embedColor = config.levelUpEmbedColor;
                let embedDesc = antonymsLevelUp(config.levelUpEmbedDesc, message, level);
                ...
                let channelID = config.levelUpChannelID;
        ...
        console.log(colorize.ansify(` #red[An unexpected error occurred when managing chat levelling: \n${error}]`))
    }
}


function antonymsLevelUp(string, message, level) {
    return string
